...
public:
    ...
    UPROPERTY() TArray<USceneComponent*> InteriorShell;
    ...
    TArray<FVector> WalkSamples;
    TArray<float> WalkWidths;
    UPROPERTY() TArray<UTextRenderComponent*> AtlasLabels;
    ...
    bool HeartStudy=false;
    bool Adventure=false;
    UPROPERTY() USceneComponent* StressCloud;
    UPROPERTY() TArray<USceneComponent*> CareLights;
    TArray<FVector> AdventureTravel;
    TArray<TArray<FVector>> AdventureCareRoutes;
    int32 AdventureRouteIndex=0;
    float ReleaseTime=0;
    bool GardenPreview=false, GardenFocused=false, GardenRestored=false;
    int32 GardenDelivered=0;
    uint32 GardenStarMask=7;
    float GardenFocusPull=0;
    UPROPERTY() TArray<UStaticMeshComponent*> GardenStars;
    UPROPERTY() TArray<UStaticMeshComponent*> GardenRoots;
    UPROPERTY() TArray<UMaterialInstanceDynamic*> GardenTreeMaterials;
    TArray<FVector> GardenStarPlaces;
    int32 GardenNearest(FVector P) const;
    bool GardenBridgeHeight(FVector P,float& Height) const;
    void TickGarden(float Dt);
    int32 HeartVariant=0;
    FString StudyTitle;
    FVector StudySpawn=FVector(-1810,210,63),StudyWallPoint=FVector::ZeroVector;
    TArray<FVector> StudyRoute;
    UPROPERTY() USceneComponent* PulseSeed;
    UPROPERTY() TArray<UStaticMeshComponent*> StudyMeshes;
    int32 Carry=0, ThoughtCount=4, CareCount=0;
    uint32 ThoughtMask=15;
    ...
    void MakeHeartStudy();
    void MakeAdventure();
    void TickAdventure(float Dt);
    void AdventureCare(int32 Index);
    FString AdventurePlace(FVector P) const;
    void MakeAbdomen();
...
public:
    ...
    UPROPERTY() UStaticMeshComponent* CarryStar;
    ...
    void RunHeartSmoke(float Dt);
    void RunAdventureSmoke(float Dt);
    void RunGardenSmoke(float Dt);
    void CaptureAdventure(float Dt);
...
public:
    ...
    void DrawHeartStudy(ABodyPawn* Pawn);
    void DrawAdventure(ABodyPawn* Pawn);
