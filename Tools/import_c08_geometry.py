...
manifest=json.loads((art/'asset-manifest.json').read_text());changed=json.loads((art/'Source/C08/changed-assets.json').read_text())
for e in manifest:
...
(art/'material-readback-c08.json').write_text(json.dumps(report,indent=2))
for key in ['Cortex','Moss','Earth','Leaf','Stone']:
...
unreal.log('C08_GEOMETRY_IMPORTED')
