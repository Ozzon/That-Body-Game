...
walls=[]
for zone in [dict(key='Pool',center=[1070,55],radii=[580,530])]:
...
parts=[];fx,fy=FOCUS;fz=float(height(fx,fy))
...
plant_groups=[[],[],[],[]];rocks=[]
beds=[(-1570,-560,280,160),(400,-1570,340,140),(-320,1820,350,140)]
...
parts=[];lights=[]
for i,(x,y) in enumerate([(-1770,290),(780,-1190),(1060,1410)]):
...
parts=[];start=np.array(ROOT_START,dtype=float);end=np.array(ROOT_END,dtype=float);dr=end-start;side=np.array([-dr[1],dr[0]])/np.linalg.norm(dr)
...
# Source review includes Attention at native character scale, outside the export manifest.
...
for e in exports:
    ...
    # Landscapes grow wider; the tree, lotus and thoughts retain human-scale proportions.
    key=e['key']
...
area('Large soft morning key',(-3800,-3600,6500),270000000,4400,(1,.86,.77));area('Lavender fill',(2100,4100,4700),180000000,4100,(.72,.79,1));area('Golden rim',(3400,-2500,4900),200000000,2800,(1,.67,.40),(0,0,600))
...
camera_data=bpy.data.cameras.new('Brain garden full level');camera=bpy.data.objects.new('Brain garden full level',camera_data);stage.objects.link(camera);camera.location=(-9500,-550,8100);target=Vector((0,0,320));camera.rotation_euler=(target-camera.location).to_track_quat('-Z','Y').to_euler();camera_data.type='ORTHO';camera_data.ortho_scale=9650;camera_data.clip_end=40000;scene.camera=camera
...
camera.location=(-3350,-2350,2700);target=Vector((-120,-200,660));camera.rotation_euler=(target-camera.location).to_track_quat('-Z','Y').to_euler();camera_data.ortho_scale=3850
