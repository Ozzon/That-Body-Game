...
part=np.where(mid[:,2]<220,0,1+(mid[:,0]>0).astype(int)*2+(mid[:,1]>0).astype(int))
for i,name in enumerate(['Ground','CortexFrontRight','CortexFrontLeft','CortexBackRight','CortexBackLeft']):
    f=F[part==i];ids,inv=np.unique(f.ravel(),return_inverse=True);o=mesh('SM_BrainGarden_'+name,V[ids].tolist(),inv.reshape(-1,3).tolist(),C[ids],cortex,surface,N[ids].tolist());add(o,'Cortex')
...
add(merge(parts,'SM_BrainGarden_TreeTrunk'),'TreeTrunk',['Interactive'])

# Carved light channels follow the main trunk, recalling the reference's spiral bark marks.
parts=[]
for j in range(4):
    pts=[]
    for t in np.linspace(0,1,14):
        z=170+t*650;a=j*math.pi*.5+.70*math.sin(t*4);r=145-60*t;pts.append((math.cos(a)*r+25*math.sin(t*5),math.sin(a)*r,z))
    parts.append(sweep('Bark light inlay',pts,[3.8,3.2,2.2],(.82,.38,.065),tree,glow,10,2,.01))
...
# Hand-shaped radial stonework leaves a generous clear path around the roots.
stone=(.42,.285,.15);stones=[]
for ring,(r,w,z,n) in enumerate([(382,65,79,27),(457,78,70,32),(551,95,64,38),(650,94,64,44)]):
    for i in range(n):
        a=(i+.5)*math.tau/n+ring*.045;x,y=r*math.cos(a),r*math.sin(a);length=math.tau*r/n-3
        stones.append(block('Tree court stone',(x,y,z),(w,length,25),np.array(stone)*(1+rng.uniform(-.09,.09)),garden,surface,a,6,i+ring*20))
add(merge(stones,'SM_BrainGarden_TreeCourt'),'TreeCourt',['Interactive'])

# Continuous path foundations eliminate gaps at junctions and beneath the pavers.
foundations=[]
for path in PATHS:
    pts=curve(path['points'],24,False);vv=[];ff=[]
    for i,p in enumerate(pts):
        dr=pts[min(i+1,len(pts)-1)]-pts[max(0,i-1)];side=np.array([-dr[1],dr[0]])/np.linalg.norm(dr)
        for s in [-1,1]:
            q=p+side*(path['width']/2+2)*s;bd,bh=bridge_values(q[0],q[1]);zz=float(bh)-25 if bd<0 else float(height(*q))+1
            vv.append((q[0],q[1],zz))
        if i:a=(i-1)*2;ff.append((a,a+2,a+3,a+1))
    foundations.append(mesh('Quiet path foundation',vv,ff,(.32,.215,.125),garden,surface))
add(merge(foundations,'SM_BrainGarden_PathFoundations'),'Paths')
# Curved walking routes are built from individual beveled pavers, with quiet joints.
road_stones=[];previous=[]
for path in PATHS:
    pts=curve(path['points'],12,False);segments=np.linalg.norm(np.diff(pts,axis=0),axis=1);total=segments.sum();cum=np.r_[0,np.cumsum(segments)];count=max(1,int(total/99));width=path['width'];rows=3
    for i in range(count):
        t=(i+.5)*total/count;k=min(len(pts)-2,max(0,np.searchsorted(cum,t)-1));f=(t-cum[k])/segments[k];p=pts[k]*(1-f)+pts[k+1]*f;forward=pts[k+1]-pts[k];forward/=np.linalg.norm(forward);side=np.array([-forward[1],forward[0]]);angle=math.atan2(forward[1],forward[0])
        for row in range(rows):
            q=p+side*((row-1)*width/3);r=np.linalg.norm(q)
            bd,_=bridge_values(q[0],q[1])
            if 412<r<712 or inner_distance(*q)>-60 or waters(*q)<25 or bd<0:continue
            if any(distance(np.array([q[0]]),np.array([q[1]]),old,False)[0]<oldw/2-25 for old,oldw in previous):continue
            z=float(height(*q))+8;road_stones.append(block('Path flagstone',(q[0],q[1],z),(total/count-2,width/3-2,17),np.array(stone)*(1+rng.uniform(-.035,.035)),garden,surface,angle,3,i*7+row))
    previous.append((pts,width))
add(merge(road_stones,'SM_BrainGarden_CurvedPaving'),'Paths')
...
V=[tuple(p) for p in poly];F=[tuple(p if isinstance(p,int) else look[tuple(p)] for p in t) for t in tri];C=[(.045,.34,.39)]*len(V)
...
for i,p in enumerate(pts):
    ...
    for f in [-1,-.72,.72,1]:
        q=p+side*86*f;V.append((q[0],q[1],float(height(*q))-35));C.append((.055,.30,.33) if abs(f)>.8 else (.07,.47,.50))
    if i:
...
add(merge(parts,'SM_BrainGarden_FocusLotus'),'Focus',['Interactive'])
...
scene.render.engine='CYCLES';scene.cycles.samples=48;scene.cycles.use_denoising=True;scene.render.resolution_x=1600;scene.render.resolution_y=1200;scene.render.resolution_percentage=100;scene.view_settings.view_transform='AgX';scene.render.image_settings.file_format='PNG';scene.render.filepath=str(ART/'Brain-garden-source.png')
